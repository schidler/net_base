...
int main(int argc, char *argv[]) {
    int i, maxi, listenfd, connfd, sockfd, epfd, nfds, port;
    ssize_t n;
    char line[MAXLINE];
    socklen_t clilen;
    ...
    ev.events = EPOLLIN | EPOLLET;      //设置要处理的事件类型   //ev.events=EPOLLIN;
    ...
    for ( ; ; ) {
        ...
        for(i = 0; i < nfds; ++i) {     //处理所发生的所有事件
            if(events[i].data.fd == listenfd) {//如果新监测到一个SOCKET用户连接到了绑定的SOCKET端口，建立新的连接。
                //初始时，监听的fd只有listenfd一个，随着连接越来越多，epool监听的fd数目由listenfd逐渐扩展
                ...
                ev.events = EPOLLIN | EPOLLET;  //设置用于注测的读操作事件   //ev.events=EPOLLIN;
